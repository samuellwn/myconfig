...
local have_uwsm = os.execute("command -v uwsm >/dev/null && uwsm check is-active")
local os_id = "unknown"

for l in io.lines("/etc/os-release") do
	local result = string.match(l, "^ID=(.*)$")
	if result then
		os_id = result
	end
end

-- Horribly ugly way to handle the possible quoting in /etc/os-release
if os_id == 'gentoo' or os_id == '"gentoo"' or os_id == "'gentoo'" then
	os_id = 'gentoo'
end

local function add_uwsm(cmd)
	if have_uwsm then
		return "uwsm app -- " .. cmd
	else
		return cmd
...
hl.on("hyprland.start", function()
	if os_id == 'gentoo' then
		...
		if os_id ~= 'manjaro' and os_id ~= 'arch' then
			hl.notification.create({
				text = "Unknown OS id '" .. os_id .. "'. May have missing functionality",
				icon = "error",
				timeout = 0,
			})
		end
	...
	hl.exec_cmd(add_uwsm("playerctld daemon"))
	hl.exec_cmd(add_uwsm("quickshell"))
end)
...
if have_uwsm then
	...
	hl.bind("WIN + Q", hl.dsp.exit())
end
hl.bind("WIN + SHIFT + Q", hl.dsp.exit())
-- hl.bind("WIN + SHIFT + S", hl.dsp.exec_cmd(add_uwsm("wezterm start")))
...
hl.bind("WIN + F", hl.dsp.exec_cmd(add_uwsm("neovide")))
hl.bind("WIN + S", hl.dsp.exec_cmd(add_uwsm("wezterm start --cwd .")))
if os_id == "gentoo" then
	hl.bind("WIN + R", hl.dsp.exec_cmd(add_uwsm("flatpak run com.brave.Browser")))
else
	hl.bind("WIN + R", hl.dsp.exec_cmd("uwsm app -- brave-browser.desktop:new-window"))
end
hl.bind("WIN + D", hl.dsp.exec_cmd("~/.config/hypr/launcher.sh"))
--[[ broken
hl.bind("WIN + D", function()
	local run
	local cmd = "wofi --show drun --define=drun-print_command=true"
	if have_uwsm then
		cmd = "wofi --show drun --define=drun-print_desktop_file=true"
	end
	...
	if p then
		...
		hl.notification.create({ text = "cannot run wofi: " .. err, icon = "error", timeout = 0 })
		return
	end

	if have_uwsm then
		run = string.gsub(run, "(%.desktop) ", "%1:", 1)
		hl.exec_cmd("uwsm app -- " .. run)
	else
		hl.exec_cmd(run)
	end
end)
